...
void otterus_core::Scripting::UserDataBinder::CreateLuaUserData(sol::state& lua)
{
	...
	lua.new_usertype<UserData>(
		...
		"create_user_data", [](const sol::table& data, sol::this_state s) -> sol::object {
			...
			if (!data.valid())
			{
				OTTERUS_ASSERT(false && "Provided user data is not valid.");
		...
		"get_user_data", [](UserData& userData, sol::this_state s) {
			const auto maybe_any = InvokeMetaFunction(
				static_cast<entt::id_type>(userData.typeId),
				"get_user_data"_hs,
				userData, s
			);
