...
namespace otterus_core::Scripting {
	...
	inline auto create_user_data(const sol::table& data, sol::this_state& s) 
	{
		...
		userData.typeId = entt::type_hash<DATA>::value();
		
		return sol::make_reference(s, userData);
	}

	template <typename DATA>
	inline auto set_user_data(otterus_physics::UserData* userData, const sol::table& data, sol::this_state& s)
	{
		auto newData = data.valid() ? data.as<DATA>() : DATA{};
	...
	template <typename DATA>
	inline auto get_user_data(otterus_physics::UserData& userData, sol::this_state& s)
	{
