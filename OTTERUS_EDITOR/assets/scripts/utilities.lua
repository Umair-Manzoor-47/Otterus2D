...
function Tileset:GetTileStartXY( id )
	assert(self:TileIdExists(id), "Tile [" ..id.. "] does not exists in TileSet ["..self.name .."]")
...
function LoadMap( mapDef )
	...
	-- Loop through layers and create Tiles
	...
	for k, v in ipairs(mapDef.layers) do
		...
		for row=0, rows do
			for col=1,cols do
				...
				-- which tileset this ID belongs to
