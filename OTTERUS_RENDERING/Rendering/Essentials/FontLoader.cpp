...
namespace otterus_rendering {
    ...
    std::shared_ptr<Font> FontLoader::Create(const std::string& fontPath, float fontSize, int width, int height)
    {
        ...
        if (fontStream.fail()) 
        {
            OTTERUS_ERROR("Failed to load font [{}] - Unable to read buffer!", fontPath);
            return nullptr;
        }
    ...
    std::shared_ptr<Font> FontLoader::CreateFromMemory(const unsigned char* fontPath, float fontSize, int width, int height)
    {
